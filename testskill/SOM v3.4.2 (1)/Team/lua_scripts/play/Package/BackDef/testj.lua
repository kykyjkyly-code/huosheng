...
["deng1"]={
    ...
    switch=function()
        if Cbuf_cnt(CBall2RoleDist("Receiver")<20,100) then
            return "pass"
        end
    end,
    Receiver=task.ReceiverTask("1pget"),
...
-- Receiver 执行 1pass 传球任务。
-- Kicker 执行 1jie 接球/辅助任务。
...
["pass"]={
  ...
  Receiver=task.ReceiverTask("1pass"),
  Kicker=task.KickerTask("1jie"),
}, 
...
["1shoot"]={
    ...
    Receiver=task.GotoPos("Receiver",-170,-100,0),
    Kicker=task.KickerTask("1shootping"),
}, 
...
["back_get"]={
    ...
    Receiver=task.ReceiverTask("1pget"),
    Kicker=task.GotoPos("Kicker",80,100,dir2),
},
...
["bdeng"]={
    ...
    switch=function()
        if Cbuf_cnt(CBall2RoleDist("Receiver")<30,50) or Cbuf_cnt(true,50) then
            return "bpass"
        end
    end,
    Receiver=task.ReceiverTask("1pget"),
...
["bpass2"]={
    ...
    Kicker=task.KickerTask("1pass"),
    Receiver=task.ReceiverTask("1jie"),
},
...
["bdeng2"]={
     switch=function()
        if CBall2RoleDist("Receiver")<30 and CBall2RoleDist("Kicker")>20 and Cbuf_cnt(true,100) then
            return "bsget"
        end
    end,
    Receiver=task.ReceiverTask("1jie"),
...
["b1shoot"]={
    ...
    Receiver=task.ReceiverTask("1shootping"),
    Kicker=task.RobotHalt("Kicker"),
}, 
