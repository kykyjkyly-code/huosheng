...
local clamp = function(v, minv, maxv)
    if v() < minv() then
        return minv()
    elseif v() > maxv() then
        return maxv()
...
local receX = function()
    ...
    -- 后场可以多往前送一点，前场别太靠近禁区
    if isBackHalf() then
        return clamp(ballx+ 130, -200,200)
    else
...
-- 让无球接应队员站在动态接球点
local KickerGoRecePos = function()
    return task.GotoPos("Kicker", receX, receY, Kickerdir2Receiverdir)
end

local ReceiverGoRecePos = function()
    return task.GotoPos("Receiver", receX, receY, rdk)
end
