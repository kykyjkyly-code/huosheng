...
["deng1"]={
    ...
    switch=function()
        if Cbuf_cnt(CBall2RoleDist("Receiver")<25,75) then
            return "pass"
        end
    end,
    Receiver=task.ReceiverTask("1pget"),
...
["pass"]={
  ...
  Receiver=task.ReceiverTask("passtiao"),
  Kicker=task.KickerTask("3jie"),
}, 
...
["sget"]={ 
    switch=function() 
        if Cbuf_cnt(CBall2RoleDist("Kicker")<20,100) then  
        return "1shoot"          
        end 
    end,
    Receiver=task.GotoPos("Receiver",-170,-100,0),
...
["1shoot"]={
    ...
    Receiver=task.GotoPos("Receiver",-170,-100,0),
    Kicker=task.KickerTask("1shoottiao"),
}, 
...
["b1shoot"]={
    ...
    Kicker=task.RobotHalt("Kicker"),
    Receiver=task.ReceiverTask("1shoottiao"),
