...
class GameState {
	...
	static const int RESTART = (KICKOFF | PENALTY | DIRECT | INDIRECT | PLACE);
	int state;
...
//阵型时碰撞到球，敌方认为比赛已经开始开始抢球，而己方还在等待裁判盒的开球指令才进入比赛。现象是：敌方已经在抢球进攻，己方待在原地不动。
	void transition(char ref_command, bool ball_kicked) {
		...
		if (state == GAME_OFF) {
			...
			case COMM_PLACE_BLUE:
				state = PLACE | BLUE | READY; return;
			case COMM_PLACE_YELLOW:
				state = PLACE | YELLOW | READY; return;
			default: break;
			}
		}
	}
	...
	bool placeBall() const{ return (state & PLACE); }
	bool ourPlaceBall() const{ return placeBall() && (state & color); }
	bool theirPlaceBall() const{ return placeBall() && !(state & color); }
