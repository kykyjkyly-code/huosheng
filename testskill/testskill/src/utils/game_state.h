...
//阵型时碰撞到球，敌方认为比赛已经开始开始抢球，而己方还在等待裁判盒的开球指令才进入比赛。现象是：敌方已经在抢球进攻，己方待在原地不动。
	...
	bool ourRestart() const{ return restart() && ((state & color) != 0); }
	bool theirRestart() const{ return restart() && ((state & color) == 0); }

	bool kickoff() const{ return (state & KICKOFF) != 0; }
	bool ourKickoff() const{ return kickoff() && ((state & color) != 0); }
	bool theirKickoff() const{ return kickoff() && ((state & color) == 0); }

	bool penaltyKick() const{ return (state & PENALTY) != 0; }
	bool ourPenaltyKick() const{ return penaltyKick() && ((state & color) != 0); }
	bool theirPenaltyKick() const{ return penaltyKick() && ((state & color) == 0); }

	bool directKick() const{ return (state & DIRECT) != 0; }
	bool ourDirectKick() const{ return directKick() && ((state & color) != 0); }
	bool theirDirectKick() const{ return directKick() && ((state & color) == 0); }

	bool indirectKick() const{ return (state & INDIRECT) != 0; }
	bool ourIndirectKick() const{ return indirectKick() && ((state & color) != 0); }
	bool theirIndirectKick() const{ return indirectKick() && ((state & color) == 0); }
	...
	bool allowedNearBall() const{ return gameOn() || ((state & color) != 0); }

	bool canKickBall() const{ return gameOn() || (ourRestart() && ((state & READY) != 0)); }
	
	bool canEitherKickBall() const { return gameOn() || (restart() && ((state & READY) != 0)); }
